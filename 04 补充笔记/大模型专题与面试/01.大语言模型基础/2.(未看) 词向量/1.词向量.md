...
> Run these examples from the repository root; saved models go to `outputs/chapter-04/word2vec/`.
...
Path("outputs/chapter-04/word2vec").mkdir(parents=True, exist_ok=True)
model.save("outputs/chapter-04/word2vec/word2vec_atec")
...
```python
# 先加载已有模型
model = gensim.models.Word2Vec.load("outputs/chapter-04/word2vec/word2vec_atec")
...
# 保存增量训练后的新模型
model.save("outputs/chapter-04/word2vec/word2vec_atec_incremental")
