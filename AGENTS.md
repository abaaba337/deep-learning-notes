...
- 输入大数据放根 datasets，训练权重放根 models，日志、图表、预测、临时 checkpoint 等生成产物放根 outputs；只提交说明及模型清单，不提交可重跑产物。
- 模型移动前后校验 SHA-256，同步 models/manifest.json；数据压缩包删除前验证解压内容。不要提交安装包、虚拟环境、缓存、凭据或个人编辑器配置。
